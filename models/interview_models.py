...
# ===== SIMPLIFIED INTERVIEW PLAN MODELS =====

class SimplifiedQuestion(BaseModel):
    """Simplified interview question with only essential information."""
    question_text: str


class SimplifiedInterviewSection(BaseModel):
    """Simplified interview section with only essential information."""
    phase: InterviewPhase
    section_name: str
    description: str
    estimated_duration_minutes: int = Field(ge=5, le=60)
    questions: List[SimplifiedQuestion] = Field(default_factory=list)


class SimplifiedInterviewPlan(BaseModel):
    """Simplified interview plan with only essential information."""
    # Plan metadata
    plan_id: str = Field(default_factory=lambda: f"interview_plan_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    created_timestamp: datetime = Field(default_factory=datetime.now)
    ...
    # Interview structure
    interview_sections: List[SimplifiedInterviewSection] = Field(default_factory=list)
    total_estimated_duration_minutes: int = 60
...
# ===== SIMPLIFIED RESUME ANALYSIS MODELS =====

class SimplifiedPersonalInfo(BaseModel):
    """Simplified personal information from resume."""
    full_name: Optional[str] = None
    email: Optional[str] = None
    location: Optional[str] = None


class SimplifiedTechnicalSkill(BaseModel):
    """Simplified technical skill with essential information only."""
    skill_name: str
    proficiency_level: Optional[SkillLevel] = None
    years_experience: Optional[int] = None


class SimplifiedWorkExperience(BaseModel):
    """Simplified work experience entry."""
    job_title: str
    company_name: str
    duration: str
...
class SimplifiedEducation(BaseModel):
...
class SimplifiedResumeAnalysis(BaseModel):
    """Simplified resume analysis with only essential information."""
    document_type: str = "resume"
    personal_info: SimplifiedPersonalInfo
    technical_skills: List[SimplifiedTechnicalSkill] = Field(default_factory=list)
    work_experience: List[SimplifiedWorkExperience] = Field(default_factory=list)
